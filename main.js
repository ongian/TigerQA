...
const QAPage = async(campaignID) => {
    ...
    try{
        let productArray = [];
        let EDPs = [];
        const response = await fetch(`https://tony-stark-abangers.herokuapp.com/https://www.tigerdirect.com/applications/campaigns/deals.asp?campaignid=${campaignID}`, {cache: "no-cache"});
...
const QASku = async(edp) => {
    try {
        const response = await fetch(`https://tony-stark-abangers.herokuapp.com/https://www.tigerdirect.com/applications/searchtools/item-Details.asp?EdpNo=${edp}`, {cache: "no-cache"});
